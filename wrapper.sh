...
if [ ! -d "yoctolib_python" ]; then
        git clone https://github.com/yoctopuce/yoctolib_python.git yoctolib_python 2>&1 >>${LOG}
        cd yoctolib_python                                                         2>&1 >>${LOG}
        git checkout "v1.10.beta"                                                  2>&1 >>${LOG}
        cd ..                                                                      2>&1 >>${LOG}
fi
...
${DIR}/collectd-yoctopuce.py | tee -a "${LOG}"
